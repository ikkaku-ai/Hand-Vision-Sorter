...
- その他: 
  - インカメラ（FaceTimeカメラ）を使用する。
  - ハンドトラッキングの精度を高めるため、明るい室内でのプレイを推奨

# 重要
## アプリの開き方
本アプリはmacアプリであるため、macOSのセキュリティ制限により、
初回起動時にブロックされる場合がある。その際は以下の手順で実行する。

【手順A：右クリックでの起動】
1. 「Hand Vision Sorter.app」を右クリック（またはControlキーを押しながらクリック）する。
2. メニューから「開く」を選択。
3. 「開発元を確認できないため開けません」という警告が出ますが、再度「開く」をクリックする。

【手順B：上記で開けない場合の最終手段】
右クリックで「開く」を選択しても起動しない場合は、以下の設定を行う。
1. 画面左上の Appleメニュー ＞「システム設定」を開く。
2. 「プライバシーとセキュリティ」を選択。
3. 右側を一番下までスクロールし、「セキュリティ」セクションを確認する。
4. 「"Hand Vision Sorter.app"は開発元を確認できないため、使用がブロックされました」
   という表示の横にある「そのまま開く」ボタンをクリックする。
5. Macのパスワードを入力し、最後に確認画面で「開く」を押すと起動する。

※カメラへのアクセス許可を求められた場合は「OK」を選択。
...
* 文字の改行設定のなしやカメラの視野角が可視化されていないので、画面全体が見やすいように全画面で遊ぶことをお勧めする
